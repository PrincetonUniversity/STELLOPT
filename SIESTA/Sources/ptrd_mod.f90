...
!         ------------
          ...
          call assert(info.eq.0,'pdtrdf: pdgetrf return info=',info)
     ...
     &                    Ck,ic,jc,desc, info)
             call assert(info.eq.0,'pdtrdf: pdgetrs return info=',info)
          endif
...
!	-----------------------------------------------------
	...
	call assert(info.eq.0,'pdtrds: descinit return info=',info)

	ineed = max(1,Locp*Locq)
	allocate( Rrhs(ineed,nblock),stat=ierr)
	call assert(ierr.eq.0,'pdtrds: alloc Rrhs,ierr=',ierr)
        Rrhs = 0.0d0
...
!         ------------
     ...
     &                  Rrhsk,irk,jrk,descRrhs,info)
          call assert(info.eq.0,'pdtrds: pdgetrs return info=',info)
...
!	----------------
	...
	call assert(ierr.eq.0,'pdtrds: dealloc Rhs ',ierr)
