...
!
      IF (ictrl_prec2d.gt.0 .and. ncurr.eq.1) THEN
         ...
         DO jl = nsmin, nsmax
            flsc(ni, mj, jl) = -t1*(buco(jl) - icurv(jl))
         END DO
      END IF
...
!      frzl_array(:,:,nsmin:nsmax,:) = 2*frzl_array(:,:,nsmin:nsmax,:)
      ...
      tomnspa_time = tomnspa_time + (skstoff - skston)
...
#if defined(SKS)
      CALL second0(skstoff)
      s_tomnsps_time = s_tomnsps_time + (skstoff-skston)
#endif
...
!      frzl_array = 2*frzl_array
      ...
      DEALLOCATE (work1)
      CALL second0(skstoff)
      s_tomnspa_time = s_tomnspa_time + (skstoff - skston)
