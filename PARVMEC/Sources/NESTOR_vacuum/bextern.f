#if defined(SKS)      
      SUBROUTINE bextern_par(plascur, wint,lscreen)
      USE vacmod
...
!
      ...
      CALL becoil_par(r1b,z1b,brad,bphi,bz,bvac(1,1),bvac(1,2),
     1                bvac(1,3),lscreen)
...
!
      CALL tolicu_par (plascur)
      CALL belicu_par (bexu, bexv, bexn, cosuv, sinuv, r1b, z1b)
...
!     NOTE: BEXN == NP*F = -B0 dot [Xu cross Xv] NP        (see PKM, Eq. 2.13)
      ...
      bextern_time = bextern_time + (skstoff - skston)

      END SUBROUTINE bextern_par
#endif
...
!     NOTE: BEXN == NP*F = -B0 dot [Xu cross Xv] NP        (see PKM, Eq. 2.13)
!
      bexni(:nuv3) = wint(ns:nuv3*ns:ns)*bexn(:nuv3)*pi2*pi2

#if defined(SKS)
      CALL second0(skstoff)
      s_bextern_time = s_bextern_time + (skstoff - skston)
#endif
