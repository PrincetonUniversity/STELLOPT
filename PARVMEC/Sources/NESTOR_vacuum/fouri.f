#if defined(SKS)      
      SUBROUTINE fouri_par(grpmn, gsource, amatrix, amatsq, bvec, 
     1                     bvecNS, ndim)
      USE vacmod
...
C-----------------------------------------------
      ...
      REAL(rprec), DIMENSION(mnpd2,nuv3), INTENT(in) :: grpmn
      ...
      REAL(rprec), DIMENSION(mnpd,ndim), INTENT(inout) :: bvec, bvecNS
C-----------------------------------------------
...
!
      ...
      ELSE
         ...
         DO i = 1, nu2
            DO j = 1, nv
               k = k + 1
               IF (nuv3min.LE.k .AND. k.LE.nuv3max)
     1            source(k) = p5*onp*(gsource(k) - gsource(imirr(k)))
            END DO
         END DO
      END IF
...
!      bvec(1:mn0-mf1:mf1,:) = 0                                 !Zero m=0,n<0

!
!     ZERO AMATRIX(0,n,m',n') M = 0 MODES FOR n < 0             
!
!      amatrix(1:mn0-mf1:mf1,:,1:ndim*ndim) = 0                 !Moved to fourp SPH08215
!SANITY CHECKS
...
!
      ...
      fouri_time = fouri_time + (skstoff - skston)

      END SUBROUTINE fouri_par
#endif
...
C-----------------------------------------------
      ...
      REAL(rprec), DIMENSION(nuv3) :: wint
C-----------------------------------------------
C   L o c a l   P a r a m e t e r s
C-----------------------------------------------
C     interior  (int_ext=-1), exterior  (int_ext=+1)  neumann problem
...
C   L o c a l   V a r i a b l e s
C-----------------------------------------------
      INTEGER :: k, i, j, n, kvi, kui, mn, m, mn0, isym, l, q
...
#if defined(SKS)
      CALL second0(skstoff)
      s_fouri_time = s_fouri_time + (skstoff - skston)
#endif
