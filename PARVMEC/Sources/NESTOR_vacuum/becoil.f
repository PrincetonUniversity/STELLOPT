#if defined(SKS)      
      SUBROUTINE becoil_par(rad, zee, br, bp, bz, brvac, bpvac, bzvac,
     1                      lscreen)
...
C-----------------------------------------------
      REAL(rprec), DIMENSION(nuv3), INTENT(in) :: rad, zee
      REAL(rprec), DIMENSION(nuv3), INTENT(out) :: br, bp, bz
...
!
      ...
      becoil_time = becoil_time + (skstoff - skston)
      END SUBROUTINE becoil_par
#endif      
...
#if defined(SKS)
      CALL second0(skstoff)
      s_becoil_time = s_becoil_time + (skstoff - skston)
#endif
