 ...
 IF(MAL.GT.0.AND.QN.AND..NOT.PLOT_XYZ) THEN
  ...
  IF(FAST_IONS) THEN
     ...
     DO is=1,ns_b
        ...
        END IF
     END DO
     CALL MPI_ALLREDUCE(MPI_IN_PLACE,Bmax_b,ns_b,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ierr)
     CALL MPI_ALLREDUCE(MPI_IN_PLACE,Bmin_b,ns_b,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ierr)
#else
     DO is=1,ns_b
        CALL INTERPOLATE_FIELD(s_b(is),.TRUE.)
        CALL FIND_BMIN_BMAX(is,MAL,MAL,.TRUE.)
...
!------------------------------------------------------------------------------------------- 
  ...
  IF(iostat.EQ.0) THEN
     ...
     DO m=0,mpolbd
        DO n=-ntorbd,ntorbd
           IF(m.EQ.0.AND.n.LT.0) CYCLE
           IF(ABS(borbic_add(n,m)).LT.PREC_B.OR.ABS(borbis_add(n,m)).LT.PREC_B) CYCLE
           DO imn=1,mnboz_b
