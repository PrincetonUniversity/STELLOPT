...
!-----------------------------------------------------------------------
      MODULE torlines_write_par
      CONTAINS
      SUBROUTINE torlines_write1d_parhdf5(n1,n2,mystart,myend,var_name,INTVAR,FLTVAR,DBLVAR)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
      USE mpi_params
      USE hdf5
      USE torlines_runtime, ONLY: id_string, handle_err, &
                                 MPI_BARRIER_ERR, nprocs_torlines
      USE mpi_inc
!-----------------------------------------------------------------------
!     Input Variables
...
!-----------------------------------------------------------------------
      ...
      ! Open file
      CALL h5fopen_f('torlines_'//TRIM(id_string)//'.h5', H5F_ACC_RDWR_F, file_id, ier, access_prp = fapl_id)
      ...
      DO i = 0, nprocs_torlines-1
         IF (myworkid == i) THEN
            ...
            ! Open file
            CALL h5fopen_f('torlines_'//TRIM(id_string)//'.h5', H5F_ACC_RDWR_F, file_id, ier, access_prp = H5P_DEFAULT_F)
         ...
         IF (ierr_mpi /=0) CALL handle_err(MPI_BARRIER_ERR,'torlines_write_parhdf5',ierr_mpi)
      END DO
...
!     End Subroutine
!-----------------------------------------------------------------------
      END SUBROUTINE torlines_write1d_parhdf5

      SUBROUTINE torlines_write2d_parhdf5(n1,n2,m1,m2,mystart,myend,var_name,INTVAR,FLTVAR,DBLVAR)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
      USE mpi_params
...
!     End Subroutine
!-----------------------------------------------------------------------
      END SUBROUTINE torlines_write2d_parhdf5
      END MODULE torlines_write_par