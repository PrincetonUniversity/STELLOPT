...
!DEC$ IF DEFINED (STELZIP)
         ...
         CALL EXECUTE_COMMAND_LINE("zip -r thrift_files.zip thrift_*.h5 ambipolar* fluxes_vs_Er* wout* DKES_coeffs* plasma_solver* travis_out*",WAIT=.TRUE.,EXITSTAT=ier,CMDSTAT=ierr_mpi,CMDMSG=cmdtxt)
         WRITE(6,*) ' zip: EXITSTAT=',ier,' CMDSTAT=',ierr_mpi; CALL FLUSH(6)
         WRITE(6,*) '     MESSAGE: ',TRIM(cmdtxt); CALL FLUSH(6)
         CALL EXECUTE_COMMAND_LINE("rm -rf wout* temp_input* ambipolar* fluxes_vs_Er* DKES_coeffs* travis_out*",WAIT=.TRUE.,EXITSTAT=ier,CMDSTAT=ierr_mpi,CMDMSG=cmdtxt)
         WRITE(6,*) ' rm: EXITSTAT=',ier,' CMDSTAT=',ierr_mpi; CALL FLUSH(6)
         WRITE(6,*) '     MESSAGE: ',TRIM(cmdtxt); CALL FLUSH(6)
         ier = 0; ierr_mpi=0
!DEC$ ENDIF
