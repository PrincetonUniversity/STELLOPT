...
!                    for the PIES code.  It also contains the
...
!          lideal       Logical to control Ideal MHD (flattening of pressure)
...
!----------------------------------------------------------------------
      IMPLICIT NONE
      LOGICAL         :: lverb, ldone, lrestart, lwout, lasym, lfreeb, &
                         lideal
      INTEGER         :: iter, nextcur, extsurfs, lastsurf, vsurf, isurf
      CHARACTER(256)  :: id_string, mgrid_file
      ...
      REAL(rprec), PARAMETER :: PIES_VERSION = 2.0
!-----------------------------------------------------------------------
!     Subroutines
...
!-----------------------------------------------------------------------
      ...
      ELSEIF (error_num == D05ADF_ERR) THEN
            WRITE(6,*) '  PIES ENCOUNTERED A NAG ERROR (D05ADF)'
            WRITE(6,*) '     CALLING FUNCTION ',TRIM(string_val)
            WRITE(6,*) '     IERR:      ',ierr
            IF (ierr == 1) THEN
      ...
      ELSEIF (error_num == C05AJF_ERR) THEN
            WRITE(6,*) '  PIES ENCOUNTERED A NAG ERROR (C05AJF)'
            WRITE(6,*) '     CALLING FUNCTION ',TRIM(string_val)
            WRITE(6,*) '     IERR:      ',ierr
            IF (ierr == 1) THEN
      ...
      ELSEIF (error_num == E04CCF_ERR) THEN
            WRITE(6,*) '  PIES ENCOUNTERED A NAG ERROR (E04CCF)'
            WRITE(6,*) '     CALLING FUNCTION ',TRIM(string_val)
            WRITE(6,*) '     IERR:      ',ierr
            IF (ierr == 1) THEN
      ...
      ELSEIF (error_num == HDF5_ERR) THEN
            WRITE(6,*) '  PIES ENCOUNTERED AN HDF ERROR'
            WRITE(6,*) '  ROUTINE:   ',TRIM(string_val)
            WRITE(6,*) '  IERR:      ',ierr
      ELSE
           WRITE(6,*) '  PIES ENCOUNTERED A GENERIC ERROR'
