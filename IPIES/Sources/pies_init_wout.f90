...
!-----------------------------------------------------------------------
      ...
      INTEGER :: bcs1(2)
      REAL(rprec) :: val1, val2, dval, scale, rhomax, pi2
      REAL(rprec), ALLOCATABLE :: rreal_in(:,:,:), zreal_in(:,:,:)
      REAL(rprec), ALLOCATABLE :: bureal_in(:,:,:), bvreal_in(:,:,:)
      REAL(rprec), ALLOCATABLE :: rho_vmec(:), rho_ext(:), ftemp(:)
...
!-----------------------------------------------------------------------
      ...
      vsurf = k - extsurfs
      iotamn = MINVAL(iotaf(:))
      iotamn = iotamn - iotamn*0.1
      iotamx = MAXVAL(iotaf(:))
      iotamx = iotamx + iotamx*0.1
      ...
      ALLOCATE(rreal_in(1:ns,1:nu,1:nv),zreal_in(1:ns,1:nu,1:nv),STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'R_in Z_in (real)',ier)
      ...
      ALLOCATE(bureal_in(1:ns,1:nu,1:nv),bvreal_in(1:ns,1:nu,1:nv),STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'B_in (real)',ier)
      ! Now initizlie modes
      mn = 1
      DO i=0,m
         DO j=-n,n
         ...
         END DO
      END DO
      FORALL(ik = 0:k) rho(ik) = (REAL(ik) / REAL(vsurf))**2
      DO ik = 1,k
         rho_vmec(ik) = SQRT(REAL(ik-1)/REAL(k-1))
      END DO
      ...
      END DO
      bsupumnc_in(:,ns) = 2.0*bsupumnc_in(:,ns-1) - bsupumnc_in(:,ns-2)
      bsupvmnc_in(:,ns) = 2.0*bsupvmnc_in(:,ns-1) - bsupvmnc_in(:,ns-2)
      ...
      DO mn = 1, mnmax_in
         ...
         f_spl%isHermite = 1
         f_spl%x1 = rho_vmec
         ftemp(:) = bsupumnc_in(mn,:)*rho_vmec**(-xm_in(mn)/2+1)
         ftemp(1) = 0.0
         CALL EZspline_setup(f_spl,ftemp,ier)
         IF (ier /= 0) CALL handle_err(EZSPLINE_ERR,'EZspline_setup/pies_init_wout',ier)
         DO ik = 1, vsurf
         ...
         bsupumnc_temp(mn,0) = bsupumnc_in(mn,1)
         CALL EZspline_free(f_spl,ier)
         !BSUPVMNC
         CALL EZspline_init(f_spl,ns,bcs1,ier)
         IF (ier /= 0) CALL handle_err(EZSPLINE_ERR,'EZspline_init/pies_init_wout',ier)
         f_spl%isHermite = 1
         f_spl%x1 = rho_vmec
         ftemp(:) = bsupvmnc_in(mn,:)*rho_vmec**(-xm_in(mn)/2+1)
         ftemp(1) = 0.0
         CALL EZspline_setup(f_spl,ftemp,ier)
         IF (ier /= 0) CALL handle_err(EZSPLINE_ERR,'EZspline_setup/pies_init_wout',ier)
         DO ik = 1, vsurf
         ...
         bsupvmnc_temp(mn,0) = bsupvmnc_in(mn,1)
         CALL EZspline_free(f_spl,ier)
      END DO
      DEALLOCATE(ftemp)
      ...
      ALLOCATE(press(0:k),iprime(0:k),iota(0:k),STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'press iprime',ier)
      ...
      IF (ier /= 0) CALL handle_err(EZSPLINE_ERR,'EZspline_setup/pies_init_wout',ier)
      CALL EZspline_setup(ip_spl,jcurv/ABS(MAXVAL(jcurv)),ier)
      IF (ier /= 0) CALL handle_err(EZSPLINE_ERR,'EZspline_setup/pies_init_wout',ier)
      DO ik = 0, vsurf
      ...
      ! Now transform to realspace (for surface extrapolation and Virtual casing)
      rreal=0.0
      zreal=0.0
      rreal_in=0.0
      zreal_in=0.0
...
!            rmns = 0; zmnc = 0
      ...
      IF (lfreeb .and. extsurfs > 0) THEN
         ...
         bumnc(:,vsurf+1:k) = bvmnc_temp(:,vsurf+1:k)
         DEALLOCATE(bsmns_temp,bumnc_temp,bvmnc_temp)
      END IF
      FORALL(ik = 0:k) rho(ik) = REAL(ik) / REAL(k)
      ! Now Fourier Transform all quantities
      !CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,rmnc,xm,xn,rreal,0,1)
      !CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,zmns,xm,xn,zreal,1,0)
      !CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bsmns,xm,xn,bsreal,1,0)
      !CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bumnc,xm,xn,bureal,0,0)
      !CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bvmnc,xm,xn,bvreal,0,0)
      !IF (lasym) THEN
      !   CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,rmns,xm,xn,rreal,1,0)
      !   CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,zmnc,xm,xn,zreal,0,0)
      !   CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bsmnc,xm,xn,bsreal,0,0)
      !   CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bumns,xm,xn,bureal,1,0)
      !   CALL uvtomn(0,k,mnmax,nu,nv,xu,xv,bvmns,xm,xn,bvreal,1,0)
      !END IF
      ! Allocate some arrays for later use
      ALLOCATE(jsreal(0:k,1:nu,1:nv),jureal(0:k,1:nu,1:nv),jvreal(0:k,1:nu,1:nv), STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'JSREAL JUREAL JVREAL (jperp)',ier)
      ALLOCATE(jsmns(1:mnmax,0:k),STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'JSMNS',ier)
      ALLOCATE(jumnc(1:mnmax,0:k),jvmnc(1:mnmax,0:k),STAT=ier)
      IF (ier /= 0) CALL handle_err(ALLOC_ERR,'JUMNC JVMNC',ier)
      IF (lasym) THEN
         ALLOCATE(jsmnc(1:mnmax,0:k),STAT=ier)
         IF (ier /= 0) CALL handle_err(ALLOC_ERR,'JSMNC',ier)
         ALLOCATE(jumns(1:mnmax,0:k),jvmns(1:mnmax,0:k),STAT=ier)
         IF (ier /= 0) CALL handle_err(ALLOC_ERR,'JUMNS JVMNS',ier)
      END IF
!-----------------------------------------------------------------------
!     End Subroutine
!-----------------------------------------------------------------------
