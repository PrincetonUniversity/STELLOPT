...
!-----------------------------------------------
      ALLOCATE (work(ns*nzeta,4), stat=i)
...
!
     ...
     1                     + work(jk,2)*sinmu(i,m))*faccon(m)
      ...
      SUBROUTINE alias_old(gcons, ztemp, gcs, gsc, gcc, gss)
      USE vmec_main
      IMPLICIT NONE
C-----------------------------------------------
...
C-----------------------------------------------
     ...
     1               cosnv(k,n)
               ...
               ELSE
               DO js = 2,ns
                  gcs(js,n,m) = gcs(js,n,m) + tcon(js)*sinnv(k,n)*
     1               (work(js+l,1)-work(js+l,3))
                  gsc(js,n,m) = gsc(js,n,m) + tcon(js)*cosnv(k,n)*
     1               (work(js+l,2)-work(js+l,4))
                  gss(js,n,m) = gss(js,n,m) + tcon(js)*sinnv(k,n)*
     1               (work(js+l,2)+work(js+l,4))
                  gcc(js,n,m) = gcc(js,n,m) + tcon(js)*cosnv(k,n)*
     1               (work(js+l,1)+work(js+l,3))
               END DO
               END IF
            END DO
