...
# PENTA Class
class PRESSURE_SOLVER_IMPLICIT_SOURCE_EXPLICIT:
    ...
    def set_source(self,species,source_type, total_power=None, sigma_r=None, fraction_alpha_heating=None, cte_source=None):
        ...
        match source_type:
            ...
            case 'external_gaussian':
                if((total_power is None) or (sigma_r is None)):
                    print('ERROR: Need to provide total_power [W] and sigma_r [m] for gaussian external source')
                    exit(1) 
                else:
                    self.sources[species][source_type] = {'total_power' : total_power, 'sigma_r' : sigma_r }
    ...
    def get_sources(self,species: str,rho_grid, it):
        ...
        for source_type in self.sources[species]:
            ...
            match source_type:
                ...
                case 'external_gaussian':
                    r0 = 0.0
                    sigma_r = self.sources[species]['external_gaussian']['sigma_r']
                ...
                case 'Coll_Heat_Exchange':
                    ...
                    for ir,rho in enumerate(rho_grid):
                        ...
                        for species2 in self.list_of_species:
                            ...
                            # aux_source[ir] += 1.29*EC*(n2*T2-n1*T1)
                            # aux_source[ir] += 1.29*EC*(n2*self.T[species2][it-1,ir] - n1*self.T[species][it-1,ir])
                            # aux_source[ir] += collisions.collisionalHeatExchange(n1,self.T[species][it-1,ir],m1,Z1,n2,self.T[species2][it-1,ir],m2,Z2)    
    ...
    def get_LHS_tridig_matrix(self,species,it):
        ...
        D = np.zeros(self.Nr)
        D[0] = -self.theta * Q(drho) * dr * 0.5  / (self.P[species][it,1]-self.P[species][it,0])
        D[1:] = -self.theta * Q(self.rho_grid[1:]) / dpdr(self.rho_grid[1:])  ## NOT GOOD AT r=0, hence why like this
    ...
    def get_RHS_vector(self,species,it):
        ...
        g = self.P[species][it-1,:] + (2./3)*self.dt*self.total_sources[species][it,:]
    ...
    def update_at_start(self):
        ...
        info_str = f'  {self.time[0]:<13.2f}{1:<10}{self.P['electrons'][0,0]/1E6:<18.2E}{self.total_sources['electrons'][0,0]/1E6:<20.2E}{self.P['deuterium'][0,0]/1E6:<18.2E}{self.total_sources['deuterium'][0,0]/1E6:<21.2E}{0.0:<13.2E}'
        print(info_str)
