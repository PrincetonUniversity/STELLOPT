...
class MyApp(QMainWindow):
	...
	def update_plot(self,i):
		...
		if (plot_name == 'Summary'):
			...
			if (self.ui.rho_button.isChecked()):
				theta_temp = 360.*float(self.u)/float(self.nu-1)
				phi_temp = 360.*float(self.v)/float(self.nv2*self.vmec_data.nfp)
				self.ax.plot(self.nflux,val[:,self.u,self.v])
				self.ax.set_xlabel('Normalized Flux')
				self.ax.set_title(rf"${plot_name}$ (u,v={theta_temp:.2f}$^o$, {phi_temp:.2f}$^o$)")
			elif (self.ui.pol_button.isChecked()):
				phi_temp = 360.*float(self.v)/float(self.nv2*self.vmec_data.nfp)
				s_temp = float(self.s)/float(self.ns-1)
				u = np.linspace(0.,360.,self.nu)
				self.ax.plot(u,val[self.s,:,self.v])
				self.ax.set_xlabel(rf'Poloidal Angle [$^o$]')
				self.ax.set_title(rf"${plot_name}$ (s,v={s_temp:.2f}, {phi_temp:.2f}$^o$)")
			elif (self.ui.tor_button.isChecked()):
				theta_temp = 360.*float(self.u)/float(self.nu-1)
				s_temp = float(self.s)/float(self.ns-1)
				v = np.linspace(0.,360./self.vmec_data.nfp,self.nv2+1)
				self.ax.plot(v,val[self.s,self.u,0:self.nv2+1])
				self.ax.set_xlabel(rf'Toroidal Angle [$^o$]')
				self.ax.set_title(rf"${plot_name}$ (s,u={s_temp:.2f}, {theta_temp:.2f}$^o$)")
			elif (self.ui.flux_button.isChecked()):
				s_temp = float(self.s)/float(self.ns-1)
				rho_temp = np.sqrt(s_temp)
				u = np.linspace(0.,360.,self.nu)
				v = np.linspace(0.,360./self.vmec_data.nfp,self.nv2+1)
				self.ax.pcolormesh(v,u,np.squeeze(val[self.s,:,0:self.nv2+1]),cmap='jet',shading='gouraud')
				self.ax.set_xlabel('Toroidal Angle (v) [$^o$]')
				self.ax.set_ylabel('Poloidal Angle (u) [$^o$]')
				self.ax.set_title(rf"${plot_name}$ (s={s_temp:.2f}, r/a= {rho_temp:.2f})")
			elif (self.ui.poltor_button.isChecked()):
				theta_temp = 360.*float(self.u)/float(self.nu-1)
				s = np.linspace(0.,1.0,self.ns)
				v = np.linspace(0.,360./self.vmec_data.nfp,self.nv2+1)
				self.ax.pcolormesh(v,s,np.squeeze(val[:,self.u,0:self.nv2+1]),cmap='jet',shading='gouraud')
				self.ax.set_xlabel('Toroidal Angle [deg]')
				self.ax.set_ylabel('Normalized Flux (s)')
				self.ax.set_title(rf"${plot_name}$ (u={theta_temp:.2f}$^o$)")
			elif (self.ui.RZ_button.isChecked()):
				phi_temp = 360.*float(self.v)/float(self.nv2*self.vmec_data.nfp)
				...
				self.ax.set_ylabel('Z [m]')
				self.ax.set_aspect('equal')
				self.ax.set_title(rf"${plot_name}$ (v={phi_temp:.2f}$^o$)")
			elif (self.ui.ThreeD_button.isChecked()):
				self.canvas.hide()
				self.vtkWidget.show()
