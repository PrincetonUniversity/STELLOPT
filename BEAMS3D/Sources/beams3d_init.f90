...
SUBROUTINE beams3d_init
   ...
   IF (lfidasim) THEN
      ...
      IF (zmin_fida == 0.0) zmin_fida = zmin
      IF (phimin_fida == 0.0) phimin_fida = phimin
      IF (rmax_fida == 0.0) rmax_fida = rmax
      IF (zmax_fida == 0.0) zmax_fida = zmax
      IF (phimax_fida == 0.0) phimax_fida = phimax
      IF (nr_fida == 0) nr_fida = nr
      IF (nphi_fida == 0) nphi_fida = nphi
      IF (nz_fida == 0) nz_fida = nz
      ! nenergy_fida = ns_prof4 !should stay this way!
      ! npitch_fida = ns_prof5
      IF (nenergy_fida == 0) nenergy_fida = ns_prof4
      IF (npitch_fida == 0) npitch_fida = ns_prof5
   END IF
