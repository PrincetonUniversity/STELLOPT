...
!-----------------------------------------------------------------------
   ...
   IF (lfidasim) THEN
      ...
      IF (nenergy_fida .eq. 1) THEN
         e_h = 1.0/( 0.5 * MAXVAL(mass) * partvmax * partvmax /1.60217662E-19 / 1000.0) !(energy_fida(nenergy_fida) - energy_fida(1)) / (nenergy_fida)
      ELSE 
         e_h = 1.0/( energy_fida(2) - energy_fida(1)) !(energy_fida(nenergy_fida) - energy_fida(1)) / (nenergy_fida)
      END IF
