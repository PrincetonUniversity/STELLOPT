            ...
            DO i=1,nr_fida
               DO k = 1, nz_fida
                  DO j=1,nphi_fida
                     ...
                     j3 = MAX(MIN(CEILING( x0*h3_prof           ), ns_prof3-1), 1) ! V Bin, because dist has full toroidal revolution, TODO: CHECK -1
   ...
   DO WHILE (istat == 0)
     ...
     CALL TOLOWER(TRIM(line))
      IF ( line == '&fidasim_inputs_b3d') THEN
         ...
         EXIT
      END IF
      IF (istat > 0) CALL handle_err(NAMELIST_PRESENT_ERR,'fidasim_inputs_b3d in: input.'//TRIM(id_string),istat)
   END DO
   rewind(iunit)
