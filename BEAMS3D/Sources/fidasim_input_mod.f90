   ...
   SUBROUTINE beams3d_write_fidasim(write_type)
      ...
      IF (myworkid == master) THEN
         SELECT CASE (TRIM(write_type))
            ...
            END IF
            CLOSE(iunit)
            IF (equilibrium_file == 'none')   equilibrium_file = TRIM(result_dir) // TRIM(runid) //  '_equilibrium.h5'   ! ! File containing plasma parameters and fields
            IF (geometry_file == 'none')  geometry_file = TRIM(result_dir) // TRIM(runid) //  '_geometry.h5'      ! ! File containing NBI and diagnostic geometry
            IF (distribution_file == 'none') distribution_file = TRIM(result_dir) // TRIM(runid) //  '_distribution.h5'      ! ! File containing fast-ion distribution
            IF (neutrals_file == 'none') neutrals_file = TRIM(result_dir) // TRIM(runid) //  '_neutrals.h5'
