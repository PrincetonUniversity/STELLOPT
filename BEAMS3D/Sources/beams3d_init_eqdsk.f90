...
!-----------------------------------------------------------------------
      ...
      USE beams3d_grid, ONLY: raxis_g => raxis, phiaxis, &
                                 ...
                                 s_max,s_max_te, s_max_ne,s_max_zeff,s_max_ti, s_max_pot, s_max_omeg
...
!-----------------------------------------------------------------------
      ...
      REAL(rprec) :: brtemp, bptemp, bztemp, betatot, sflx, uflx, &
                     tetemp,netemp,titemp,zetemp,pottemp, omegtemp,rsmax, rsmin,&
...
!-----------------------------------------------------------------------
      ...
      IF (mylocalid == mylocalmaster) THEN
         TE = 0; NE = 0; TI=0; S_ARR=1.5; U_ARR=0; POT_ARR=0; ZEFF_ARR = 1;OMEG_ARR = 1;
      END IF
#if defined(MPI_OPT)
      ...
      IF (sflx <= s_max) THEN
         ...
         END IF
            ...
            POT_ARR(i,:,k) = pottemp;  OMEG_ARR(i,:,k) = omegtemp; ZEFF_ARR(i,:,k) = zetemp
      ELSE
         pottemp = 0; sflx = 1
