...
CONTAINS
   ...
   SUBROUTINE handle_err(error_num, string_val, ierr)
      ...
      IF (error_num /= MPI_CHECK) WRITE(6, *) '!!!!! ERROR !!!!!'
      ...
      ELSEIF (error_num == C05AJF_ERR) THEN
         ...
         ELSE
            WRITE(6, *) '     UNKNOWN NAG ERROR!'
         END IF
      ELSEIF (error_num == LSODE_ERR) THEN
         ...
         END IF
      ELSEIF (error_num == RKH68_ERR) THEN
      ...
      ELSEIF (error_num == HDF5_ERR) THEN
         ...
         WRITE(6, *) '  ROUTINE:   ', TRIM(string_val)
         WRITE(6, *) '  IERR:      ', ierr
      ELSEIF (error_num == BOOZER_ERR) THEN
         WRITE(6, *) '  ERROR LOADING BOOZER FILE'
         WRITE(6, *) '  ROUTINE:   ', TRIM(string_val)
         WRITE(6, *) '  IERR:      ', ierr
      ELSEIF (error_num == MPI_REDU_ERR) THEN
      ...
      ELSEIF (error_num == MPI_FINE_ERR) THEN
         ...
         CALL FLUSH(6)
         STOP
      ELSEIF (error_num == MPI_CHECK) THEN
      ELSE
      ...
      ierr_mpi = 0
      CALL MPI_BARRIER(MPI_COMM_BEAMS, ierr_mpi)
      IF (ANY(error_array /= 0)) CALL MPI_FINALIZE(ierr_mpi)
      DEALLOCATE(error_array)
      RETURN
#else
      IF (error_num == MPI_CHECK) RETURN
#endif
      print *, myworkid,' calling STOP'
      CALL FLUSH(6)
      STOP
