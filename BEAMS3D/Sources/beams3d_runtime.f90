...
MODULE beams3d_runtime
   ...
   REAL(rprec), PARAMETER :: BEAMS3D_VERSION = 3.74 ! restart_grid implemented
   !-----------------------------------------------------------------------
   !     Subroutines
...
CONTAINS
   ...
   SUBROUTINE handle_err(error_num, string_val, ierr)
      ...
      ELSEIF (error_num .eq. BAD_BEAMDEX_ERR) THEN
         WRITE(6, *) '  BEAMS3D ENCOUNTERED A NAMELIST ERROR'
