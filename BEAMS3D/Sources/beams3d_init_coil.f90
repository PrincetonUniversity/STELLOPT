...
SUBROUTINE beams3d_init_coil
   ...
   DO ik = 1, nextcur
      DO j = 1, coil_group(ik) % ncoil
         ...
         IF (j == 1) current_first = current
         IF (lraw) THEN
            coil_group(ik) % coils(j) % current = current * extcur_in(ik)
         ELSE
            IF (current_first /= zero) coil_group(ik) % coils(j) % current = (current / current_first) * extcur_in(ik)
         END IF
      END DO
   END DO
