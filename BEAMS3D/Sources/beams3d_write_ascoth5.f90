...
SUBROUTINE beams3d_write_ascoth5(write_type)
   ...
   SELECT CASE (TRIM(write_type))
      ...
      IF (myworkid == master) THEN
         ...
         CALL write_var_hdf5(qid_gid,'ORBITWRITE_TOROIDALANGLES', ier, DBLVAR = DBLE(- 1.0))
         CALL write_var_hdf5(qid_gid,'ORBITWRITE_POLOIDALANGLES', ier, DBLVAR = DBLE(- 1.0))
      ...
      itemp = 0
      IF (lbeam) d3 = 2
      itemp(myworkid + 1) = COUNT(end_state(d1:d2) < 3) ! Don't count shinethrough particles or port particles
      ...
      itemp = 0
      IF (lbeam) d3 = 2
      itemp(myworkid + 1) = COUNT(end_state(d1:d2) < 3) ! Don't count shinethrough particles
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, itemp, nprocs_beams, MPI_INTEGER, MPI_SUM, MPI_COMM_BEAMS, ierr_mpi)
      k2 = SUM(itemp(1:myworkid + 1))
      k1 = k2 - itemp(myworkid + 1) + 1
      kmax = SUM(itemp)
