...
SUBROUTINE out_beams3d_part(t, q)
    ...
    IF ((q(1) >= rmin-eps1) .and. (q(1) <= rmax+eps1) .and. &
       ...
       d4 = MAX(MIN(1+nsh_prof4+FLOOR(h4_prof*(vll_temp+omeg)), ns_prof4), 1) ! vll
       ...
       IF (lfidasim_cyl) THEN
         ...
         y0 = MAX(((q(4)+omeg)**2+vperp**2),1.0)
         d4 = MIN(MAX(CEILING((y0*E_by_v-emin_fida)*e_h),1),nenergy_fida)
         d5 = MIN(MAX(CEILING(((q(4)+omeg)/SQRT(y0)-pimin_fida)*pi_h),1),npitch_fida)
