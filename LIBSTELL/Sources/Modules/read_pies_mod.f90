...
!     Get the Variables
      ...
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      status=nf_get_vara_double(ncid,IOTAID,iotaf)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      status=nf_get_vara_double(ncid,PSIID,phi)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      status=nf_get_vara_double(ncid,XID,x)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      status=nf_get_vara_double(ncid,RELBUPID,relbup)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      status=nf_get_vara_double(ncid,BXBYID,bxby)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
