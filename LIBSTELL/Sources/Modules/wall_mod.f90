...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine LINE_BOX_INTERSECTION(x1, y1, z1, x2, y2, z2, xmin, ymin, zmin, xmax, ymax, zmax, intersects)
        ! This was generated by CHAT GPT so boosh
         implicit none
        logical :: intersects
        DOUBLE PRECISION :: x1, y1, z1, x2, y2, z2, xmin, ymin, zmin, xmax, ymax, zmax, tmin, tmax

        intersects = .false.

        ! Calculate the minimum and maximum values of t for each axis
        tmin = max((xmin - x1) / (x2 - x1), (xmax - x1) / (x2 - x1))
        tmax = min((xmin - x1) / (x2 - x1), (xmax - x1) / (x2 - x1))

        tmin = max(tmin, max((ymin - y1) / (y2 - y1), (ymax - y1) / (y2 - y1)))
        tmax = min(tmax, min((ymin - y1) / (y2 - y1), (ymax - y1) / (y2 - y1)))

        tmin = max(tmin, max((zmin - z1) / (z2 - z1), (zmax - z1) / (z2 - z1)))
        tmax = min(tmax, min((zmin - z1) / (z2 - z1), (zmax - z1) / (z2 - z1)))
