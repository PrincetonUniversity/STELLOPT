...
!        IF (lprint) PRINT *,'CALL PEEK'        
        ...
        IF (icount.EQ.1 .OR. MOD(icount,10).EQ.0) THEN
           ...
           END IF
           IF (lprint) PRINT 910, kout, fsq_nl, xmod, xmax, fsq_lin
        END IF
...
!-----------------------------------------------
     ...
     &            fsq_min_lin
      ...
      LOGICAL  :: lactive = .TRUE., lprint, lStore
!-----------------------------------------------
!
