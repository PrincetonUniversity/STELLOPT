...
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
  510    CONTINUE
         ewti = rwork(lewt+i-1)
         CALL xerrwv (
     1      'lsode--  at t (=r1), ewt(i1) has become r2 .le. 0.', 50,
...
c-----------------------------------------------------------------------
  ...
  602 CONTINUE
     ...
     1   itask, 0, 0, zero, zero)
      GOTO 700
  603 CONTINUE
      CALL xerrwv ('lsode--  istate .gt. 1 but lsode not initialized  '
     1   , 50, 3, 0, 0, 0, 0, 0, zero, zero)
      GOTO 700
  604 CONTINUE
      CALL xerrwv ('lsode--  neq (=i1) .lt. 1     ', 30, 4, 0, 1,
  ...
  608 CONTINUE
     ...
     1   , 0, zero, zero)
      GOTO 700
  609 CONTINUE
      CALL xerrwv ('lsode--  ml (=i1) illegal.. .lt.0 or .ge.neq (=i2)'
     1   , 50, 9, 0, 2, ml, neq(1), 0, zero, zero)
      GOTO 700
  610 CONTINUE
      CALL xerrwv ('lsode--  mu (=i1) illegal.. .lt.0 or .ge.neq (=i2)'
     1   , 50, 10, 0, 2, mu, neq(1), 0, zero, zero)
      GOTO 700
  611 CONTINUE
      CALL xerrwv ('lsode--  maxord (=i1) .lt. 0  ', 30, 11, 0, 1,
     1   maxord, 0, 0, zero, zero)
      GOTO 700
  612 CONTINUE
      CALL xerrwv ('lsode--  mxstep (=i1) .lt. 0  ', 30, 12, 0, 1,
     1   mxstep, 0, 0, zero, zero)
      GOTO 700
  613 CONTINUE
      CALL xerrwv ('lsode--  mxhnil (=i1) .lt. 0  ', 30, 13, 0, 1,
  ...
  614 CONTINUE
     ...
     1   , 50, 14, 0, 0, 0, 0, 1, h0, zero)
      GOTO 700
  615 CONTINUE
      CALL xerrwv ('lsode--  hmax (=r1) .lt. 0.0  ', 30, 15, 0, 0, 0, 0
     1   , 1, hmax, zero)
      GOTO 700
  616 CONTINUE
      CALL xerrwv ('lsode--  hmin (=r1) .lt. 0.0  ', 30, 16, 0, 0, 0, 0
     1   , 1, hmin, zero)
      GOTO 700
  617 CONTINUE
      CALL xerrwv (
  ...
  618 CONTINUE
     ...
     2   60, 18, 0, 2, leniw, liw, 0, zero, zero)
      GOTO 700
  619 CONTINUE
      CALL xerrwv ('lsode--  rtol(i1) is r1 .lt. 0.0        ', 40, 19,
     1   0, 1, i, 0, 1, rtoli, zero)
      GOTO 700
  620 CONTINUE
      CALL xerrwv ('lsode--  atol(i1) is r1 .lt. 0.0        ', 40, 20,
     1   0, 1, i, 0, 1, atoli, zero)
      GOTO 700
  621 CONTINUE
      ewti = rwork(lewt+i-1)
      CALL xerrwv ('lsode--  ewt(i1) is r1 .le. 0.0         ', 40, 21,
