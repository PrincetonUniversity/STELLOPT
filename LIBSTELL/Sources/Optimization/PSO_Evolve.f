...
!----------------------------------------------------------------------
     ...
     1          x_global(n), x_personal(n),
     2          vel(n), temp_fvec(m), vmax(n), stat=ierr)
      ...
      IF (myid .eq. master) THEN
         ...
         DO i=2,NP
            DO j = 1, n
               ...
               x_array(i,j)=XCmin(j)+rand_C1*(XCmax(j)-XCmin(j))
            END DO
         END DO
      END IF
...
!DEC$ ENDIF
     ...
     1                    '           CHI_BEST'
               WRITE(6,'(70("="))')
            END IF
            IF (fnorm == 0.0) fnorm = 1E10 ! Do this so we won't think it's a minimum
            CALL MPI_BARRIER(MPI_COMM_STEL, ierr)
            IF (ierr .ne. 0) CALL mpi_stel_abort(ierr)
     ...
     1                       nfeval,myid,fnorm**2,fnorm_personal**2
         END IF
         CALL FLUSH(6)
...
!DEC$ IF DEFINED (MPI_OPT)
     ...
     1                          1, MPI_REAL8, MPI_COMM_STEL, ierr)
         IF (ierr .ne. 0) CALL mpi_stel_abort(ierr)
     ...
     1                      '---  New global minimum at iteration',
     2                         nfeval,' Chi-Squared:',fnorm_global**2
               WRITE(6,'(A)') '   '
            END IF
         END IF
         CALL MPI_BARRIER(MPI_COMM_STEL, ierr)
...
!DEC$ ENDIF
         ...
         CALL random_number(rand_C1)
         vel = vel + c1*(x_personal-x_temp)*rand_C1
     1             + c2*(x_global-x_temp)*rand_C1
         DO i = 1, n
            IF ((ABS(vel(i)) > vmax(i))) vel(i) = sign(vmax(i),vel(i))
         END DO
         
         ! Kick global to new spot
