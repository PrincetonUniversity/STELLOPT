C     -----------
C     This file contains the modified L-BFGS_B algorithm for STELLOPT.
C     The original source code can be found at
C     http://users.iems.northwestern.edu/~nocedal/lbfgsb.html
C     The modifications here are necessary for STELLOPT integration,
C     and are not intended to modify the algorithmic behavior in any
C     other way. Changes in the code appear preceded by a line comment as follows:
C     mod-JS
C
C     The modifications are listed here: (TO BE COMPLETED)
C     1) Added USE statements for stel_kinds, only: rprec
C     2) Changed 'double precision' to REAL(rprec)
C     3) Changed the 'iteration' output to be directed to '6'
C
C     J.C. Schmitt
...
C     mod-JS
c      double precision f, factr, pgtol, x(n), l(n), u(n), g(n),
      real(rprec) f, factr, pgtol, x(n), l(n), u(n), g(n),
c
c-jlm-jn
     +                 wa(2*m*n + 5*n + 11*m*m + 8*m), dsave(29)
      ...
      !-----------------------------------------------
      REAL(rprec), EXTERNAL :: timer, dcopy
...
C     mod-JS
c      double precision f, factr, pgtol,
      real(rprec) :: f, factr, pgtol
      real(rprec) :: x(n), l(n), u(n), g(n), z(n), r(n), d(n), t(n)
      real(rprec) :: xp(n)
      real(rprec) :: wa(8*m)
      real(rprec) :: ws(n, m), wy(n, m), sy(m, m), ss(m, m)
      real(rprec) :: wt(m, m), wn(2*m, 2*m), snd(2*m, 2*m), dsave(29)
...
C     mod-JS: Commented out lines
c         itfile = 8
c         if (iprint .ge. 1) then
cc                                open a summary file 'iterate.dat'
c            open (8, file = 'iterate.dat', status = 'unknown')
c         endif            
c     mod-JS: Redefined itfile to be the same as screen output
         itfile = 6
