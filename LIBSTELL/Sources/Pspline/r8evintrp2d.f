      subroutine r8evintrp2d(xget,yget,x,nx,y,ny,jspline,
...
C
      ...
      REAL*8 f(icoeff,ixdim,iydim)        ! function data w/coefficients
C
...
C
     ...
     >     icoeff,nx,ny,ixdim,iydim,ier)
      if(ier.ne.0) return
c
      call r8herm2xy(xget,yget,x,nx,y,ny,0,0,
     >     i,j,xparam,yparam,hx,hxi,hy,hyi,ier)
      if(ier.ne.0) return
c
      call r8fvintrp2d(ict,1,1,
...
C   NO ERROR CHECKING
C
      subroutine r8fvintrp2d(ict,ivec,ivecd,
...
C
      ...
      REAL*8 fin(icoeff,ixdim,iydim)      ! data on which to interpolate...
C
...
C
      integer :: idcub,idlin
      REAL*8 :: xp,dxlin,dxlini,h,hi,ans1,ans2
      REAL*8 :: f22(2,2)
C
     ...
     >           fin,ixdim,iydim)
            ...
            do i=1,inum
               if(maxval(iderivs(1:2,i)).le.1) then
                  ...
                  ict4(indx)=1
                  call r8herm2fcn(ict4,ivec,ivecd,
