      subroutine r8akherm2(x,nx,y,ny,fherm,nf2,ilinx,iliny,ier)
C
...
!============
      ...
      REAL*8 fherm(0:3,nf2,ny)            ! data/Hermite array
C
...
C
      call r8akherm2p(x,nx,y,ny,fherm,nf2,ilinx,iliny,0,0,ier)
C
      return
      end
C----------------------------
      subroutine r8akherm2p(x,nx,y,ny,fherm,nf2,ilinx,iliny,ipx,ipy,ier)
C
C  create a data set for Hermite interpolation, based on Akima's method
C  [Hiroshi Akima, Communications of the ACM, Jan 1974, Vol. 17 No. 1]
C
...
! idecl:  explicitize implicit REAL declarations:
      ...
      REAL*8 cytrap0,cytrapn,zansr,cxm2,cxp2
!============
      integer nx,ny,nf1                 ! array dimensions
      REAL*8 x(nx)                        ! x coordinate array
      REAL*8 y(ny)                        ! y coordinate array
...
c
               call r8akherm0(cxmm,cxm,cxp,cxpp,wx,fherm(1,1,iy))
...
C
            if(nx.gt.2) then
               cxpp=(ftmp(3,iy)-ftmp(2,iy))/(xx(3)-xx(2))
               fherm(1,1,iy)=1.5_r8*cxp-0.5_r8*cxpp
C
               cxmm=(ftmp(nx-1,iy)-ftmp(nx-2,iy))/(xx(nx-1)-xx(nx-2))
               fherm(1,nx,iy)=1.5_r8*cxm-0.5_r8*cxmm
c
            else
               fherm(1,1,iy) = cxp  ! =cxm, nx=2
               fherm(1,nx,iy) = fherm(1,1,iy)
...
C
            cxtrap0=2.0_r8*fherm(1,1,iy)-cxp
            cxtrapn=2.0_r8*fherm(1,nx,iy)-cxm
C
         else
C
...
c
               call r8akherm0(cymm,cym,cyp,cypp,wy,fherm(2,ix,1))
...
C
            if(ny.gt.2) then
               cypp=(ftmp(ix,3)-ftmp(ix,2))/(yy(3)-yy(2))
               fherm(2,ix,1)=1.5_r8*cyp-0.5_r8*cypp
C
               cymm=(ftmp(ix,ny-1)-ftmp(ix,ny-2))/(yy(ny-1)-yy(ny-2))
               fherm(2,ix,ny)=1.5_r8*cym-0.5_r8*cymm
c
            else
               fherm(2,ix,1) = cyp  ! =cym, ny=2
               fherm(2,ix,ny)=fherm(2,ix,1)
...
C  extrapolate to slope to ghost points just past bdy...
C
            cytrap0=2.0_r8*fherm(2,ix,1)-cyp
            cytrapn=2.0_r8*fherm(2,ix,ny)-cym
C
         else
C
C  BC supplied by user.  Also use this for extrapolation...
