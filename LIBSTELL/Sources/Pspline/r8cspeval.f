...
c
      subroutine r8cspeval(xget,iselect,fval,x,nx,ilinx,f,ier)
c
!============
! idecl:  explicitize implicit INTEGER declarations:
      ...
      INTEGER ier,nx
!============
      REAL*8 xget                         ! interpolation target
      REAL*8 fval(3)                      ! output values
      REAL*8 x(nx),f(4,nx)                ! spline data
c
...
c--------------------------
c
      call r8cspevx(xget,x,nx,ilinx,ia(1),dxa(1),ier)
      if(ier.ne.0) return
c
      call r8cspevfn(iselect,1,1,fval,ia,dxa,f,nx)
c
      return
      end
c
...
c
      subroutine r8cspevx(xget,x,nx,ilinx,i,dx,ier)
c
!============
! idecl:  explicitize implicit INTEGER declarations:
      IMPLICIT NONE
...
c
      ...
      REAL*8 x(nx)                        ! independent coord. array
c
...
c
      ...
      REAL*8 dx                           ! displacement of target pt w/in cell
                                        ! dx = x-x(i)
c
...
c
      subroutine r8cspevfn(ict,ivec,ivd,fval,iv,dxv,f,nx)
c
c  input:
!============
! idecl:  explicitize implicit INTEGER declarations:
...
c  input:
      ...
      REAL*8 dxv(ivec)                    ! displacements w/in cell -- vectors
c
      REAL*8 f(4,nx)                      ! cubic fcn spline coeffs array
c
c  usage example:
c
...
c  evaluate df/dx
            ...
            do v=1,ivec
               i=iv(v)
               dx=dxv(v)
               fval(v,iaval)=f(2,i)+dx*(2.0_r8*f(3,i)+dx*3.0_r8*f(4,i))
            enddo
         endif
c
         if(ict(3).gt.0) then
c  evaluate d2f/dx2
            ...
            do v=1,ivec
               i=iv(v)
               dx=dxv(v)
               fval(v,iaval)=2.0_r8*f(3,i)+dx*6.0_r8*f(4,i)
            enddo
         endif
      endif
c
