...
C  f90 dynamic array
C
      real, dimension(:,:,:), allocatable :: fbicub ! bicubic subsection
      real, dimension(:,:), allocatable :: fwk ! work array
      real, dimension(:), allocatable :: bcx1,bcx2,bcy1,bcy2 ! BCs for mkbicub
c
      real, dimension(:,:,:,:), allocatable :: fcorr ! correction spline
      real, dimension(:,:), allocatable :: bcc1,bcc2 ! correction BCs
c
...
c
      ...
      bcx1=0.0; bcx2=0.0; bcy1=0.0; bcy2=0.0
c
      do iz=1,nz
...
c
     ...
     >               hz*(-2*f(4,ix,iy,1)-f(4,ix,iy,2))/6
         ...
         enddo
c
         fwk(1,1:nz)=0.0  ! values are all zero, only BC is set...
         do iy=1,ny
            do ix=1,nx
               call mkspline(z,nz,fwk,
