...
c
      subroutine r8cspline(x,nx,fspl,ibcxmin,bcxmin,ibcxmax,bcxmax,
     >   wk,iwk,ilinx,ier)
c
!============
...
! idecl:  explicitize implicit REAL declarations:
      REAL*8 half,sixth
!============
      REAL*8 x(nx)                        ! x axis (in)
      REAL*8 fspl(4,nx)                   ! spline data (in/out)
      integer ibcxmin                   ! x(1) BC flag (in, see comments)
      REAL*8 bcxmin                       ! x(1) BC data (in, see comments)
      integer ibcxmax                   ! x(nx) BC flag (in, see comments)
      REAL*8 bcxmax                       ! x(nx) BC data (in, see comments)
      REAL*8 wk(iwk)                      ! workspace of size at least nx
...
c
      call r8v_spline(ibcxmin,ibcxmax,nx,x,fspl,wk)
c
      do i=1,nx
