      subroutine r8spvec(ict,ivec,xvec,ivd,fval,nx,xpkg,fspl,iwarn,ier)
c
...
c input:
      ...
      REAL*8 fspl(4,nx)                   ! (non-compact) spline coefficients
c
c output:
...
c
      ...
      REAL*8 dxv(ivec)                    ! displacements w/in zones
c
c---------------------------------------------------------------
c
c  error checks
...
c
      iv=0
      call r8xlookup(ivec,xvec,nx,xpkg,1,iv,dxv,dxv,dxv,iwarn)
c
c  vectorized evaluation
c
      call r8cspevfn(ict,ivec,ivd,fval,iv,dxv,fspl,nx)
c
      return
      end
