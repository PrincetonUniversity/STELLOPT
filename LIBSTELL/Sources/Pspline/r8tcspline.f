...
!============
      ...
      REAL*8 bcxmin(inb1x,*),bcxmax(inb1x,*) ! inth x inph defined (if used)
      REAL*8 bcthmin(inb1th,*),bcthmax(inb1th,*) ! inx x inph defined (if used)
      REAL*8 bcphmin(inb1ph,*),bcphmax(inb1ph,*) ! inx x inth defined (if used)
c
c  input:
...
c
         do ix=1,inx
            wk(iabcth1+ix-1)=0.0_r8
            wk(iabcth2+ix-1)=0.0_r8
     ...
     >         ((ibcthmax.eq.1).or.(ibcthmax.eq.2))) then
         ...
         enddo
         do ith=1,inth
            wk(iabcx1+ith-1)=0.0_r8
            wk(iabcx2+ith-1)=0.0_r8
