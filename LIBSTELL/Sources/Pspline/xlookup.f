...
c  input:
      ...
      real xvec(ivec)                   ! x points to lookup on xpkg grid
c
      integer nx                        ! size of grid
      real xpkg(nx,4)                   ! grid data
c
...
c  output:
      ...
      real dxn(ivec)                    ! displacement w/in zone (see imode)
      real hv(ivec)                     ! zone width (if imode=2)
      real hiv(ivec)                    ! inverse zone width (if imode=2)
c
...
c
      if(nx.gt.2) then
         if(xpkg(3,4).eq.0.0) then
            ...
            ilin=0
            if(xpkg(3,4).gt.2.5) then
               ialg=3
            else if(xpkg(3,4).gt.1.5) then
               ialg=2
            else
               ialg=1
            endif
...
c
         ...
         do i=1,ivec
            if((xvec(i).lt.xpkg(1,1)).or.(xvec(i).gt.xpkg(nx,1))) then
               ...
               if(xuse(i).lt.0.0) xuse(i)=xuse(i)+period
