...
C  local...
C
      real wx(2),wy(2),wz(2),exy(2,2),exz(2,2),eyz(2,2),fxyz(2,2,2)
C
...
C
               if(nx.gt.2) then
                  cxpp=(ftmp(3,iy,iz)-ftmp(2,iy,iz))/(xx(3)-xx(2))
                  fherm(1,1,iy,iz)=1.5*cxp-0.5*cxpp
C
                  cxmm=(ftmp(nx-1,iy,iz)-ftmp(nx-2,iy,iz))/
     >                 (xx(nx-1)-xx(nx-2))
                  fherm(1,nx,iy,iz)=1.5*cxm-0.5*cxmm
c
               else
                  fherm(1,1,iy,iz)=cxp  ! =cxm, nx=2
                  fherm(1,nx,iy,iz)=fherm(1,1,iy,iz)
...
C
               cxtrap0=2.0*fherm(1,1,iy,iz)-cxp
               cxtrapn=2.0*fherm(1,nx,iy,iz)-cxm
C
            else
C
...
C
               if(ny.gt.2) then
                  cypp=(ftmp(ix,3,iz)-ftmp(ix,2,iz))/(yy(3)-yy(2))
                  fherm(2,ix,1,iz)=1.5*cyp-0.5*cypp
C
                  cymm=(ftmp(ix,ny-1,iz)-ftmp(ix,ny-2,iz))/
     >                 (yy(ny-1)-yy(ny-2))
                  fherm(2,ix,ny,iz)=1.5*cym-0.5*cymm
c
               else
                  fherm(2,ix,1,iz) = cyp  ! =cym, ny=2
                  fherm(2,ix,ny,iz)=fherm(2,ix,1,iz)
...
C  extrapolate to slope to ghost points just past bdy...
C
               cytrap0=2.0*fherm(2,ix,1,iz)-cyp
               cytrapn=2.0*fherm(2,ix,ny,iz)-cym
C
            else
C
C  BC supplied by user.  Also use this for extrapolation...
...
C
               if(nz.gt.2) then
                  czpp=(ftmp(ix,iy,3)-ftmp(ix,iy,2))/(zz(3)-zz(2))
                  fherm(3,ix,iy,1)=1.5*czp-0.5*czpp
C
                  czmm=(ftmp(ix,iy,nz-1)-ftmp(ix,iy,nz-2))/
     >                 (zz(nz-1)-zz(nz-2))
                  fherm(3,ix,iy,nz)=1.5*czm-0.5*czmm
c
               else
                  fherm(3,ix,iy,1) = czp
                  fherm(3,ix,iy,nz)=fherm(3,ix,iy,1)
...
C  extrapolate to slope to ghost points just past bdy...
C
               cztrap0=2.0*fherm(3,ix,iy,1)-czp
               cztrapn=2.0*fherm(3,ix,iy,nz)-czm
C
            else
C
C  BC supplied by user.  Also use this for extrapolation...
