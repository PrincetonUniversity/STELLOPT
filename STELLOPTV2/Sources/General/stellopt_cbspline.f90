...
!---------------------------------------------------------------
  SUBROUTINE cbspline_eval(cs, s, f, ierr)
    ...
    REAL(CBR8) :: kms, smk, c2_0, c2_1, c2_2, c1_0, c1_1
    INTEGER mu

    ierr = 0
    IF (cs%ncoefs.LT.1) THEN !Spline object is not properly initialized.
       ierr = 1
       RETURN
    ENDIF

    ! Test whether in-bounds
    IF ((s.LT.cs%knot(2)).OR.(s.GT.cs%knot(cs%ncoefs))) THEN
       ierr = 2
       RETURN
    ENDIF
    ...
    c2_0 = (kms*cs%coef(mu-4) + (s - cs%knot(mu-3))*cs%coef(mu-3))/&
    ...
    c2_2 = ((cs%knot(mu+2) - s)*cs%coef(mu-2) + &
         smk*cs%coef(mu-1))/(cs%knot(mu+2) - cs%knot(mu-1))
...
!---------------------------------------------------------------
  SUBROUTINE cbspline_derivs(cs, s, f, fp, fpp, ierr)
    ...
    d2_0 = (cs%coef(mu-3) - cs%coef(mu-4))/(cs%knot(mu) - cs%knot(mu-3))
    d2_1 = (cs%coef(mu-2) - cs%coef(mu-3))/(cs%knot(mu+1) - cs%knot(mu-2))
    d2_2 = (cs%coef(mu-1) - cs%coef(mu-2))/(cs%knot(mu+2) - cs%knot(mu-1))
