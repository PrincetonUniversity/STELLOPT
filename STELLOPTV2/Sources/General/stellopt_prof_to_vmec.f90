...
!----------------------------------------------------------------------
      ...
      REAL(rprec) :: emis_xics_temp, w3_xics_temp, phi_temp, emis_xmcts_temp
...
!      IF (dex > 4) CALL setup_prof_spline(omega_spl,dex,rho(1:dex),omega(1:dex),ier)
      ...
      IF (ANY(emis_xmcts_f /= 0.0_rprec)) lnew_diag = .true.
      ...
      IF (lnew_diag) THEN
         ...
         IF (.not. lno_file) THEN
            ...
            WRITE(iunit,*) 's     emis_xics       emis_xmcts       phi'
            DO ik = 1, new_prof
               CALL get_equil_emis_xics(ne_temp(ik),TRIM(emis_xics_type),emis_xics_temp,ier)
               CALL get_equil_emis_xmcts(ne_temp(ik),TRIM(emis_xmcts_type),emis_xmcts_temp,ier)
               CALL get_equil_phi(ne_temp(ik),TRIM(phi_type),phi_temp,ier)
               WRITE(iunit,'(4es12.3)') ne_temp(ik),emis_xics_temp,emis_xmcts_temp,phi_temp
            END DO
            CLOSE(iunit)
         END IF
         DEALLOCATE(ne_temp)
