!-----------------------------------------------------------------------
!     Subroutine:    chisq_bnmnc
!     Authors:       S. Lazerson (samuel.lazerson@gauss-fusion.com)
!     Date:          07/07/2025
!     Description:   Calculate the difference between the normal field
!                    on the equilibrium surface and zero.
!-----------------------------------------------------------------------
      SUBROUTINE chisq_bnmnc(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
      USE stellopt_runtime
      USE stellopt_targets
      USE equil_vals, ONLY: bmnc_normal_total, im_normal_total, &
            in_normal_total
...
!----------------------------------------------------------------------
      IF (iflag < 0) RETURN
      n = COUNT(sigma<bigno)
      IF (iflag == 1) WRITE(iunit_out,'(A,2(2X,I8.8))') 'BNMNC ',n,5
            ...
            DO n = -bnorm_nmax, bnorm_nmax
               IF (sigma(n,m) < bigno) THEN
                  ...
                  IF ((mn > mnmax) .and. (niter == 0)) &
                     WRITE(6,*) 'WARNING: (',n,',',m,') Exceeds MNMAX in CHISQ_BNMNC'
                  IF (mn > mnmax) THEN
      ...
      ELSE
         ...
         DO m = 0, bnorm_mmax
            DO n = -bnorm_nmax, bnorm_nmax
               IF (sigma(n,m) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) target_dex(mtargets)=jtarget_bnmnc
               END IF
            END DO
         END DO
      END IF
