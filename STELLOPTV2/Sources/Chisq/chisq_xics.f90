!-----------------------------------------------------------------------
!     Subroutine:    chisq_xics
!     Authors:       S. Lazerson (lazerson@pppl.gov)
!     Date:          05/26/2012
!     Description:   Calculate difference measured and equilibrium
!                    XICS signals.
!-----------------------------------------------------------------------
      SUBROUTINE chisq_xics(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      INTEGER ::  ik
      REAL(rprec) :: xics_val, xics_bright
...
!----------------------------------------------------------------------
      ...
      IF (iflag == 1) WRITE(iunit_out,'(A,2(2X,I3.3))') 'XICS',ik,9
      ...
      IF (niter >= 0) THEN
         DO ik = 1, nprof
            ...
            xics_val = 0.0
            CALL line_int(fcn_xics,x0,x1,xics_val)
            CALL line_int(fcn_xics_bright,x0,x1,xics_bright)
            !PRINT *,'TI_LENGTH (',ik,')',xics_length
            mtargets = mtargets + 1
            targets(mtargets) = target(ik)
            sigmas(mtargets)  = sigma(ik)
            vals(mtargets)    = xics_val/xics_bright
            IF (iflag == 1) WRITE(iunit_out,'(9ES22.12E3)') target(ik),sigma(ik),xics_val/xics_bright,x0(1),x0(2),x0(3),x1(1),x1(2),x1(3)
         END DO
      ELSE
         DO ik = 1, nprof
