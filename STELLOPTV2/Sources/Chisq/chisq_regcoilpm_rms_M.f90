!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoilpm_rms_m
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoilpm_rms_m(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      USE regcoil_variables, ONLY:  rms_m_target, nlambda, regcoilpm_nml
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      IF (iflag == 1) THEN
          ...
          END DO
          WRITE(iunit_out,'(A,2(2X,I7))') 'REGCOILPM_RMS_M ', counter, 4
      ...
      IF (niter >= 0) THEN
         ...
         DO ii = 1, mnprod_x4_rcpmds
            ...
            IF (sigma(ii) < bigno) THEN
              ...
              ! The value of the results is in the rms_m_target variable
              vals(mtargets)    = rms_m_target
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, mnprod_x4_rcpmds
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoilpm_rms_m
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoilpm_rms_m
