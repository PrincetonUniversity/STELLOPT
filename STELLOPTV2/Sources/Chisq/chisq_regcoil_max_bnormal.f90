!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoil_max_Bnormal
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoil_max_Bnormal(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOIL)
      USE regcoil_variables, ONLY:  max_Bnormal_target, nlambda, regcoil_nml
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOIL)
      IF (iflag == 1) THEN
          ...
          END DO
          WRITE(iunit_out,'(A,2(2X,I7))') 'REGCOIL_max_Bnormal ', counter, 4
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, mnprod_x4_rcws
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoil_max_Bnormal
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoil_max_Bnormal
