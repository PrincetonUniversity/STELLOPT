!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoil_d2p_dist_min
!     Authors:       J.C. Schmitt (Auburn/PPPL) (jcschmitt@auburn.edu)
!     Date:          2017-2018
!     Description:   Chisq routine(s) for REGCOILPM.
...
!                    jtarget_regcoilpm_d2p_dist_min
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoilpm_d2p_dist_min(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      USE regcoil_variables, ONLY:  dipole_plasma_dist_min_target, &
                                    nlambda, regcoilpm_nml
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      IF (iflag == 1) THEN
          ...
          WRITE(iunit_out,'(A,2(2X,I7))') &
             'REGCOILPM_DIPOLE_PLASMA_DISTANCE_MIN ', counter, 4
      ...
      IF (niter >= 0) THEN
         ...
         DO ii = 1, mnprod_x4_rcpmds
            ...
            IF (sigma(ii) < bigno) THEN
              ...
              vals(mtargets) = MIN(target(ii), dipole_plasma_dist_min_target) 
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, mnprod_x4_rcpmds
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoilpm_d2p_dist_min
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoilpm_d2p_dist_min
