!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoilpm_chi2_b
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoilpm_chi2_b(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      USE regcoil_variables, ONLY:  chi2_B_target, nlambda, regcoilpm_nml
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      IF (iflag == 1) THEN
          ...
          END DO
          WRITE(iunit_out,'(A,2(2X,I7))') 'REGCOILPM_CHI2_B ', counter, 4
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, mnprod_x4_rcpmds
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoilpm_chi2_b
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoilpm_chi2_b
