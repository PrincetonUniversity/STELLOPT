!-----------------------------------------------------------------------
!     Subroutine:    chisq_segrog
!     Authors:       S. Lazerson (lazerson@pppl.gov)
!     Date:          05/26/2012
!     Description:   Calculates fit to Rogowski magnetic diagnostics.
!-----------------------------------------------------------------------
      SUBROUTINE chisq_segrog(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
      USE stellopt_runtime
      USE stellopt_targets
      USE diagno_input_mod, ONLY: read_diagno_input, lskip_rogo
      USE safe_open_mod
!-----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      ...
      IF (niter >= 0) THEN
         ...
         iunit = 12
         CALL safe_open(iunit,ier,'diagno_seg.'//TRIM(proc_string_old),'old','formatted')
         ...
         END DO
         CLOSE(iunit)
         IF (iflag == 1) WRITE(iunit_out,'(A,2(2X,I3.3))') 'SEGROG ',dex,3
         IF (iflag == 1) WRITE(iunit_out,'(A)') 'TARGET  SIGMA  FLUX'
         DO ik = 1, nprof
            ! Reordered so we get a value for every seg rog
      ...
      ELSE
         ...
         DO ik =1, nprof
            IF (sigma(ik) < bigno) THEN
               ...
               lskip_rogo(ik) = .false.
               mtargets = mtargets + 1
               IF (niter == -2) target_dex(mtargets)=jtarget_segrog
            END IF
         END DO
      END IF
      RETURN
