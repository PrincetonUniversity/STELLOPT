!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoilpm_Bnormal_total
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoilpm_Bnormal_total(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      USE regcoil_variables, ONLY:  Bnormal_total_target, &
				 nlambda, regcoilpm_nml, ntheta_plasma, &
                                 nzeta_plasma
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOILPM)
      IF (iflag == 1) THEN
          ...
          END DO
          WRITE(iunit_out,'(A,2(2X,I7))') 'REGCOILPM_BNORMAL_TOTAL ', counter, 4
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, (nzeta_plasma * ntheta_plasma)
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoilpm_Bnormal_total
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoilpm_Bnormal_total
