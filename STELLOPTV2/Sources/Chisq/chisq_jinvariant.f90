...
!-----------------------------------------------------------------------
      ...
      ! These should probably be user controlable
!      INTEGER, PARAMETER :: NUM_EP_MU = 10
      LOGICAL, PARAMETER :: FIX_PITCH = .false.
!----------------------------------------------------------------------
!     BEGIN SUBROUTINE
!----------------------------------------------------------------------
!DEC$ IF DEFINED (JINV_OPT)
      ...
      IF (iflag == 1) WRITE(iunit_out,'(A,2(2X,I3.3))') 'J_INVARIANT ',1,7
      IF (iflag == 1) WRITE(iunit_out,'(A)') 'TARGET  SIGMA  J_INV_ALL  EPMU  NS  DEX_EP_MU  DEXJSTAR'
      IF (niter >= 0) THEN
         device = "qas"
         ...
         DO ik = 1, nsd
            ...
            DO j = 1, num_ep_mu
               ...
               DO k = 1, NumJstar
                  ...
                  IF (iflag == 1) WRITE(iunit_out,'(3ES22.12E3,3(2x,I4.4))') J_avg,sigma(ik),J_inv_all(k,j),epmu(j),ik,j,k
               END DO
            END DO
         END DO
         DEALLOCATE(J_inv_all,epmu,ljinvar)
      ELSE
         DO ik = 1, nsd
            IF (sigma(ik) >= bigno) CYCLE
            DO j =1, num_ep_mu
