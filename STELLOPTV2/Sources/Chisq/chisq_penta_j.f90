!-----------------------------------------------------------------------
!     Subroutine:    chisq_penta_j
!     Authors:       S. Lazerson (samuel.lazerson@gauss-fusion.com)
!                    A. Coelho (antonio.coelho@gauss-fusion.com)
!     Date:          03/27/2026
!     Description:   Targeting of Er using PENTA code
!-----------------------------------------------------------------------
      SUBROUTINE chisq_penta_j(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
      USE stellopt_runtime
      USE stellopt_targets
      USE equil_vals, ONLY: JBS_PENTA
...
!----------------------------------------------------------------------
      ...
      ! Print Header
      IF (iflag == 1) THEN
         ik   = COUNT(target_dex == jtarget_penta_j)
         WRITE(iunit_out,'(A,2(2X,I3.3))') 'PENTA_J ', ik, 4
      ...
      ELSE
         DO ii = 1, nsd
            ...
            lneed_penta(ii) = .TRUE.
            mtargets = mtargets + 1
            IF (niter == -2) target_dex(mtargets)=jtarget_penta_j
         END DO
         istat = 0
