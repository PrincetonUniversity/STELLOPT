!-----------------------------------------------------------------------
!     Subroutine:    chisq_regcoilpm_area_plasma
...
!
!-----------------------------------------------------------------------
      SUBROUTINE chisq_regcoilpm_area_plasma(target,sigma,niter,iflag)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOIL)
      USE regcoil_variables, ONLY:  nlambda, area_plasma, regcoilpm_nml
!DEC$ ENDIF      
!-----------------------------------------------------------------------
...
!DEC$ IF DEFINED (REGCOIL)
      IF (iflag == 1) THEN
          ...
          END DO
          WRITE(iunit_out,'(A,2(2X,I7))') 'REGCOILPM_AREA_PLASMA ', counter, 4
      ...
      IF (niter >= 0) THEN
         ...
         DO ii = 1, mnprod_x4_rcpmds
            ...
            IF (sigma(ii) < bigno) THEN
              ...
              ! The value of the result is in the area_plasma variable
              vals(mtargets)    = area_plasma
         ...
         IF (ANY(sigma < bigno)) THEN
            ...
            DO ii = 1, mnprod_x4_rcpmds
               IF (sigma(ii) < bigno) THEN
                  mtargets = mtargets + 1
                  IF (niter == -2) THEN
                     target_dex(mtargets)=jtarget_regcoilpm_area_plasma
                  END IF
               END IF
            END DO
...
!     END SUBROUTINE
!----------------------------------------------------------------------
      END SUBROUTINE chisq_regcoilpm_area_plasma
