...
!-----------------------------------------------------------------------
      SUBROUTINE stelltran_bal_update_nopc(itime)
!-----------------------------------------------------------------------
!     Libraries
!-----------------------------------------------------------------------
...
!     End Subroutine
!-----------------------------------------------------------------------
      END SUBROUTINE stelltran_bal_update_nopc
      ...
      SUBROUTINE bal_update_rhs(neq,t,y,ydot)
            ...
            CALL calc_pi_coll2(prof_length,ne(:,2),te(:,2),ti(:,2),zeff(:,2), mi, picol) ! NOTE: Change is this being constant
      ...
      SUBROUTINE bal_update_jac(neq, t, y, ml, mu, pd, nrowpd)
            ...
            REAL(rprec), DIMENSION(neq/3) :: Ge_jac, Qe_mn, Qi_mn, fn, temp, temp2, temp3
            ...
            CALL arr_fderiv2(prof_length,y(1:prof_length),dr,temp2)
            CALL arr_2fderiv2(prof_length,y(1:prof_length),dr,temp3)
            CALL calc_pi_coll2(prof_length,ne(:,2),te(:,2),ti(:,2),zeff(:,2), mi, picol)
            ...
            pd(prof_length+1,prof_length+1) = -2.*(-3.*temp1/(2.*dr) + 2.*Qe_mn(1)*y(1)/(dr**2.))/(3.*y(1)*Vp(1,2)) - fn(1)/y(1)
            ...
            pd(prof_length+1,prof_length+1) = 2.*zeff(1,2)*(-1.0*(-3.*temp1/(2.*dr) + 2.*Qi_mn(1)*y(1)/(zeff(1,2)*dr**2))/Vp(1,2))/(3.*y(1)) - fn(1)/y(1)
            ...
            DO j=2,prof_length-1
                  ...
                  ! Te
                  pd(j+prof_length,j+prof_length) = -2.*(-2.*Qe_mn(j)*y(j)/(dr**2.*Vp(j,2)))/(3.*y(j)) - fn(j)/y(j)
                  ! Ti
                  pd(j+2*prof_length,j+2*prof_length) = 2.*zeff(j,2)*(2.*Qi_mn(j)*y(j)/(dr**2.*zeff(j,2)*Vp(j,2)))/(3.*y(j)) - fn(j)/y(j)
            END DO
            ! right endpoints
            ...
            pd(2*prof_length,2*prof_length) = -2.*(3.*temp1/(2.*dr)&
                                    + 2.*Qe_mn(prof_length)*y(prof_length)/(dr**2.))/(3.*y(prof_length)*Vp(prof_length,2))&
            ...
            pd(3*prof_length,3*prof_length) = 2.*zeff(prof_length,2)*(-1.0*(3.*temp1/(2.*dr)&
            ...
            DO j=1,prof_length
                  ! te/n
                  pd(j+prof_length,j) = 2.*((picol(j) - pe_rf(j,2))/y(j)&
                                    + Qe_mn(j)*temp2(j)*temp4(j)/(Vp(j,2)*y(j)))/(3.*y(j))&
                                    + y(j+prof_length)*(fn(j)/y(j) - pd(j,j))/y(j)
                  ! ti/n
                  pd(j+2*prof_length,j) = 2.*zeff(j,2)*(-1.*picol(j)/y(j)&
