      SUBROUTINE dkes_input_prepare_memory(arg, numargs, 
     1                                     dkes_input_file, dealloc)
!
...
!
     ...
     &    ntor_w=>ntor, nfp_w=>nfp, ns_w=>ns, mnmax_w=>mnmax
      USE safe_open_mod
      USE dkes_input, ONLY: mpol, ntor, lalpha, ipmb,mmnn,meshtz,idisk,
     &   lfout, ifscl, nrun, cmul, efield, nzperiod, chip, psip, mpolb,
     &   ntorb, nvalsb, borbi, btheta, bzeta, ibbi, dkes_indata
      IMPLICIT NONE
!-----------------------------------------------
...
!     REAL(rprec), PARAMETER :: efield = 0._dp
     ...
     1  half = 0.5_dp, zero = 0
      CHARACTER*(51), PARAMETER ::
     1   banner = 'THIS IS THE DKES input prep CODE Version 1.0m'
!
...
!-----------------------------------------------
     ...
     1   abs_blocal, blocal_ordered
      ...
      REAL(rprec) iota_local, twopi
      REAL(rprec) phi_edge, bmn_test
      ...
      INTEGER :: m, n, mp, nt, mp0, nt0, mpu, ntu, 
     1  nszt, nsth, kk, jj, ii, ij, ik, im, in, iout,
...
!-----------------------------------------------
     ...
     1         MIN(max_bmns,mnboz_b)
         WRITE(*,'(" cmul = ",f10.6,", efield = ",f10.6)')cmul(1),
     1         efield(1)
...
!      WRITE (iunit,'(1x,"&dkes_indata")')
      nzperiod=nfp_b
      lalpha=legendre_modes
      nrun=1
      ibbi=1
      ! cmul is same name so already set
      ! efield is same name so already set
      ! mpolb and ntorb are same name so already set
      ! chip and psip are same name so already set
      ! btheta and bzeta are same name so already set
...
!
     ...
     1           (n_norm.ge.100)) THEN
         ...
         borbi(n_norm,m_ordered(i))=blocal_ordered(i)
...
!           for later printout:
            ...
            mmnn(1,ntor) = n-1
            mmnn(2,ntor) = jj
            ! Possibly should be ij not jj
            DO i = 1, ij
                mmnn(2+i,ntor) = ivec(i)
            END DO
            !IF (ntor <= 9) THEN
...
!            ENDIF
      ...
      ! mpol and ntor same name so already set
...
!      END IF
     ...
     >   stat=istat)
      ...
      END SUBROUTINE dkes_input_prepare_memory
