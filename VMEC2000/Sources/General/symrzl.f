...
!
     ...
     &                  zcons, r1a, rua, rva, z1a, zua, zva, lua, lva,
     &                  rcona, zcona)
      USE vmec_main
      USE parallel_include_module
      IMPLICIT NONE
      ...
      DO mpar = 0, 1
         DO i = 1 + ntheta2, ntheta1
            ...
            DO jk = 1, ns*nzeta
               ...
               rus(jk,i,mpar) = rua(jka,ir,mpar) - rus(jka,ir,mpar)
               z1s(jk,i,mpar) = z1a(jka,ir,mpar) - z1s(jka,ir,mpar)
               zus(jk,i,mpar) = zus(jka,ir,mpar) - zua(jka,ir,mpar)
               lus(jk,i,mpar) = lus(jka,ir,mpar) - lua(jka,ir,mpar)
               rcons(jk,i,mpar) = rcons(jka,ir,mpar)
     &                          - rcona(jka,ir,mpar)
               zcons(jk,i,mpar) = zcona(jka,ir,mpar)
     &                          - zcons(jka,ir,mpar)
            END DO
            IF (lthreed) THEN
               DO jk = 1, ns*nzeta
                  jka = ireflect(jk)
                  rvs(jk,i,mpar) = rva(jka,ir,mpar) - rvs(jka,ir,mpar)
